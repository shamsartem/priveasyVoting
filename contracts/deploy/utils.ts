...
export const deployPaymasterContract = async (options?: DeployContractOptions) => {
  const log = (message: string) => {
    if (!options?.silent) console.log(message);
  };

  log('Starting deployment of GeneralPaymaster...');

  const wallet = options?.wallet ?? getWallet();
  const deployer = new Deployer(hre, wallet);
  const contractArtifact = await deployer.loadArtifact('GeneralPaymaster').catch((error) => {
    if (error?.message?.includes(`Artifact for contract "GeneralPaymaster" not found.`)) {
      console.error(error.message);
      throw `⛔️ Please make sure you have compiled your contracts or specified the correct contract name!`;
    } else {
      throw error;
